...
let initial = 1000.0M
let afterYears (n:int) (initial: decimal) =
    initial * (decimal)((1.0+0.1)**n)

let afterFirstYear: decimal = 30.0M
let ``exercise 1.1`` = afterYears 3 initial
...
let isEven (number:List<int>) = 
    List.sum(number) % 2 = 0

let ``exercise 1.2`` = isEven range
